...
public class SkillController {
    ...
    @Autowired
    private SkillService imageService;
    @Autowired
    private SkillRepository imageRepository;



    @PostMapping("/skill")
    public ResponseEntity<SkillDTO> createImage(@RequestBody SkillDTO imageDTO) {

        if (imageDTO.getId() != null) {
            throw new IllegalArgumentException("A new image cannot already have an ID  : idexists");
        }
        SkillDTO result = imageService.save(imageDTO);
        return ResponseEntity.ok(result);

    }

    @PutMapping("/skill/{id}")
    public ResponseEntity<SkillDTO> updateImage(
            @PathVariable(value = "id", required = false) final Integer id,
            @RequestBody SkillDTO imageDTO ) {
        ...
        Optional<SkillDTO> result = imageService.update(imageDTO);
    ...
    public ResponseEntity<?> getAllImages(
        ...
        Page<SkillDTO> page = imageService.findAll(pageable);
        return ResponseEntity.ok().body(page.getContent());
    }

    @GetMapping("/skill/{id}")
    public ResponseEntity<?> getImage(@PathVariable Integer id) {
        Optional<SkillDTO> imageDTO = imageService.findOne(id);
